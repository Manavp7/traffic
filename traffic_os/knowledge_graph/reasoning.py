...
def why_congested(
    ...
    *,
    ...
    incoming = graph.neighbors(NodeType.JUNCTION, junction_id, edge_type=EdgeType.TO, direction="in")
    for _edge, road in incoming:
        rid = road.id
        rname = road.props.get("name", rid)
        rcong = float(road.props.get("congestion", 0.0))

        incidents = graph.neighbors(NodeType.ROAD, rid, edge_type=EdgeType.OCCURRED_ON, direction="in")
        for _e, inc in incidents:
            ...
            factors.append(
                CausalFactor(kind="road_closed" if iblocked else "incident",
                             description=desc, weight=round(weight, 3),
                             node_type=NodeType.ROAD, node_id=rid)
            )
        # upstream congestion contribution (no incident but heavy)
        if not incidents and rcong >= 60:
            factors.append(
                CausalFactor(kind="upstream_congestion",
    ...
    # signal fault
    signals = graph.neighbors(NodeType.JUNCTION, junction_id, edge_type=EdgeType.CONTROLS, direction="in")
    for _e, sig in signals:
        if sig.props.get("fault"):
            factors.append(
                CausalFactor(kind="signal_fault",
                             description=f"Signal {sig.id} malfunction at this junction",
                             weight=0.9, node_type=NodeType.SIGNAL, node_id=sig.id)
            )
    ...
    if wx is not None:
        ...
        if cf < 1.0:
            ...
            factors.append(
                CausalFactor(kind="weather",
                             description=f"{kind.replace('_', ' ').title()} reduces road capacity "
                                         f"(~{int((1 - cf) * 100)}% loss)",
                             weight=round((1.0 - cf) * 1.2, 3),
                             node_type=NodeType.WEATHER, node_id="current")
            )

    # nearby events (active around ref_ts)
    events = graph.neighbors(NodeType.JUNCTION, junction_id, edge_type=EdgeType.NEAR, direction="in")
    for _e, ev in events:
