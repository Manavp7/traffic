...
class CopilotService:
    def __init__(self, storage, intelligence, *, kg=None, prediction=None,
                 planning=None, recommendation=None):
        self.storage = storage
        self.settings: Settings = getattr(storage, "settings", None) or Settings(mode="dev")
        self.tools = CopilotTools(storage, intelligence, kg=kg, prediction=prediction,
                                  planning=planning, recommendation=recommendation)
    ...
    def _ask_deterministic(self, question: str) -> dict:
        ...
        def tagged(tool: str, result: dict) -> dict:
            return {"answer": result["answer"], "tool": tool, "data": result.get("data", {}),
                    "mode": "deterministic"}
        ...
        if any(w in q for w in ("recommend", "what should", "what do we do", "action", "do about", "suggest", "fix")):
            return tagged("recommend_actions", self.tools.recommend_actions(junction))
        if any(w in q for w in ("cost", "economic", "loss", "fuel", "money", "lakh", "crore", "co2", "emission")):
            return tagged("economic_cost", self.tools.economic_cost())
        if any(w in q for w in ("forecast", "expected", "next hour", "predict", "will traffic", "later")):
            return tagged("forecast", self.tools.forecast())
        if any(w in q for w in ("which junction", "worst", "maximum congestion", "max congestion", "bottleneck", "causes")):
    ...
    def _tool_specs(self) -> list[dict]:
        return [
            ...
            {"type": "function", "function": {"name": "recommend_actions",
             ...
             "parameters": {"type": "object", "properties": {"junction": {"type": "string"}}}}},
            {"type": "function", "function": {"name": "network_summary",
    ...
    def _ask_llm(self, question: str) -> dict:
        ...
        s = self.settings
        sys = ("You are the Traffic-OS Copilot for a city traffic command center. "
        ...
        with httpx.Client(base_url=s.llm_base_url, timeout=30) as client:
            for _ in range(4):
                resp = client.post("/chat/completions", headers=headers, json={
                    ...
                    "tools": self._tool_specs(), "tool_choice": "auto",
                }).json()
                msg = resp["choices"][0]["message"]
                calls = msg.get("tool_calls")
                if not calls:
                    return {"answer": msg.get("content", ""), "tool": used_tool,
                            "data": used_data, "mode": "llm"}
                messages.append(msg)
                for call in calls:
        ...
        return {"answer": "I could not complete the request.", "tool": used_tool,
                "data": used_data, "mode": "llm"}
