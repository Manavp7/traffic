...
class CopilotTools:
    def __init__(self, storage, intelligence, kg=None, prediction=None, planning=None, recommendation=None):
    ...
    def worst_junction(self) -> dict:
        ...
        top = spots[0]
        ans = f"The worst junction is {top.name} ({top.congestion:.0f}/100, {level(top.congestion)})."
        if bns:
            ans += f" Root bottleneck: {bns[0].name} — {bns[0].explanation}"
        return {"answer": ans, "data": {"hotspots": [vars(s) for s in spots],
    ...
    def top_hotspots(self, n: int = 5) -> dict:
        ...
        return {"answer": f"Top {len(spots)} congestion points: {lines}.",
    ...
    def accidents_count(self, period: str = "month") -> dict:
        ...
        return {"answer": f"There have been {len(accidents)} accidents in the last {period}.",
    ...
    def forecast(self, horizon_min: int = 60) -> dict:
        ...
        ans = (f"In {horizon_min} min, average congestion is forecast at {avg:.0f}/100 "
               f"with {severe} severe segment(s); worst is {worst.segment_id} "
               f"({worst.predicted_congestion:.0f}).")
        return {"answer": ans, "data": {"avg": round(avg, 1), "severe": severe,
                                        "worst_segment": worst.segment_id}}
    ...
    def accident_risk(self) -> dict:
        ...
        return {"answer": f"Highest accident-risk roads: {lines}.",
    ...
    def economic_cost(self) -> dict:
        ...
        ans = (f"Today's estimated congestion cost is {summ['cost_human']} "
    ...
    def recommend_actions(self, junction: str | None = None) -> dict:
        ...
        return {"answer": f"Recommended actions: {lines}",
    ...
    def network_summary(self) -> dict:
        ...
        ans = (f"Network average congestion is {s.get('avg_congestion', 0):.0f}/100 "
